...
class StoreInfoCommandTest: XCTestCase {
    var command = StoreInfoCommand()
    ...
    func testWrongCommad() {
        ...
        command.fetchData(callback: { result in
            ...
            case .failure(let error):
                if case let StoreInfoCommand.CommandError.serverError(err) = error {
